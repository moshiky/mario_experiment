 ...
 */
package competition.cig14.cmac;
...
//            System.out.println(Arrays.toString(coordinates));
    ...
    public static int[] GetTiles(
        ...
        int[] qstate = new int[variables.length];
	int[] base = new int[variables.length];
	int[] coordinates = new int[variables.length + hash.length + 1];   /* one interval number per rel dimension */
	int num_coordinates;

        num_coordinates = variables.length + 1 + hash.length;
        for(i=0; i<hash.length; i++){
            coordinates[variables.length + i + 1] = hash[i];
        }
...
//            System.out.println(Arrays.toString(coordinates));
    ...
    private static int hash_coordinates(int[] coordinates, int num_indices, int memory_size) {
        ...
        if (first_call) {
            for (k = 0; k < 2048; k++) {
                rndseq[k] = util.RNG.randomInt(Integer.MAX_VALUE);
...
//        variables[1] /= .25;
        ...
        int[] tiles;
        tiles = TileCoding.GetTiles(5, variables, 512, 0, 0, -1);
        System.out.println(Arrays.toString(tiles));
        tiles = TileCoding.GetTiles(5, variables, 512, 0, 10, -1);
        System.out.println(Arrays.toString(tiles));
...
//        variables[1] /= .25;
        ...
        tiles = TileCoding.GetTiles(5, variables, 512, 1, 0, -1);
        System.out.println(Arrays.toString(tiles));
        tiles = TileCoding.GetTiles(5, variables, 512, 1, 10, -1);
        System.out.println(Arrays.toString(tiles));
    }
}
