...
abstract public class AbstractionEnsembleAgent extends BasicMarioAIAgent implements Agent {
    ...
    private double[] getCustomState() {
        ...
        state[0] = this.isMarioAbleToJump ? 1 : 0;
        state[1] = this.isMarioOnGround ? 1 : 0;
        state[2] = this.marioMode;
        state[3] = this.marioFloatPos[0] < this.prevMarioPos[0] ? 0 : (this.marioFloatPos[0] == this.prevMarioPos[0] ? 1 : 2);
        state[4] = this.enemies(2, 0);
        state[5] = this.obstacle();
        int[] enemy = closestEnemy();
        if(Math.abs(enemy[0]) < 11 && Math.abs(enemy[1]) < 11){
            state[6] = enemy[0]+10;
        } else {
