...
*/
    ...
    private List<Pair<StateAction, Double>> getFullSimilarities(StateAction prevSA) {
        List<Pair<StateAction, Double>> states = new ArrayList<>();
        ...
        StateAction sa = getState();
        sa.setAction(prevSA.getAction());

        states.add(new Pair<>(sa.clone(), 1.0));
        ...
        if(true)/*simStage != 1*/{
            ...
            for (int i = 0; i < size; ++i) {
                Pair<StateAction, Double> tstatePair = states.get(i);
                ...
                if (taction <= 6) {
                    ...
                    states.add(new Pair<>(tstateAction2, strongSimFactor * tsim));
                }
            }
        }
        ...
        if(true)/*simStage != 2*/{
            ...
            for (int j = 0; j < size; ++j) {
                Pair<StateAction, Double> tstatePair = states.get(j);
                Double tsim = tstatePair.getSecond();
                StateAction tstateAction = tstatePair.getFirst().clone();
                int taction = tstateAction.getAction();
                int[] tstate = tstateAction.getState();
                ...
                states.add(new Pair<>(new StateAction(tstate, sa.getExtraState(), newtaction), weakSimFactor * tsim));
            }
        }
        ...
        if(true)/*simStage != 3*/{
            ...
            for (int j = 0; j < size; ++j) {
                ...
                states.add(new Pair<>(new StateAction(tstate, sa.getExtraState(), taction), weakSimFactor * tsim));
            }
        }
        ...
        if(true)/*simStage != 5*/{
            ...
            for (int i = 0; i < size; ++i) {
                ...
                states.add(new Pair<>(new StateAction(tstate, sa.getExtraState(), taction), strongSimFactor * tsim));
            }
        }
        ...
        if (action <= 6) {
            state[state_shoot] = state[state_shoot] == 0 ? 1 : 0;
        }*/

        // +26 action = (action + 6) % 12;

        /*ssa = new StateAction(state, sa.getExtraState(), action);
        pair = new Pair<StateAction, Double>(ssa, 0.1);
        states.add(pair);*/
